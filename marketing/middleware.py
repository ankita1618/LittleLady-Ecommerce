...
def is_offset_greater(time_string_offset):
    ...
    offset_time_formated = datetime.datetime.strptime(offset_time, "%Y-%m-%d %H:%M:%S") #change in given format
...
class DisplayMarketing(MiddlewareMixin):
    def process_request(self, request):
        ...
        elif marketing_message and message_offset is not None and is_offset_greater(message_offset):  #is greater than now
            request.session['marketing_message'] = marketing_message
        else:
            try:
